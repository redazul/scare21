...
Shader "WeatherMaker/WeatherMakerWeatherMapShader"
{
	Properties
	{
		...
		[Header(Other)]
		[NoScaleOffset] _WeatherMakerWeatherMapMaskTexture("Mask Texture", 2D) = "white" {} // mask texture
		_WeatherMakerWeatherMapMaskOffset("Mask Offset", Vector) = (0.0, 0.0, 0.0, 0.0)
	}
	SubShader
	{
		Tags { }
		...
		Pass
		{
			...
			{
				float sum = 0.0;
				sum += (simplexNoise2D(P) * weights2[0]);
				sum += (simplexNoise2D(P * 2.0) * weights2[1]);
				sum = (sum + 1.0) * 0.5;
				return sum;
			}
			...
			{
				float sum = 0.0;
				sum += (simplexNoise2D(P) * weights3[0]);
				sum += (simplexNoise2D(P * 2.0) * weights3[1]);
				sum += (simplexNoise2D(P * 4.0) * weights3[2]);
				sum = (sum + 1.0) * 0.5;
				return sum;
			}
			...
			{
				float sum = 0.0;
				sum += (simplexNoise2D(P) * weights4[0]);
				sum += (simplexNoise2D(P * 2.0) * weights4[1]);
				sum += (simplexNoise2D(P * 4.0) * weights4[2]);
				sum += (simplexNoise2D(P * 8.0) * weights4[3]);
				sum = (sum + 1.0) * 0.5;
				return sum;
			}
			...
			float GetCloudType(float2 uv, float coverage, float density)
			{
				...
				float typeNoiseOrig = NoiseFunction(samp, _CloudTypeNoiseType, _CloudTypeNoiseTypeInv) * (1.0 + (frac((coverage + density) * 0.5) * 0.15));
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float typeNoise = GetCloudType(i.uv, coverageNoise, densityNoise);
