	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 blendCloudTemporal(fixed4 prev, fixed4 cur, fixed4 diff, float4 uv, wm_full_screen_fragment i)
			{
                ...
                UNITY_BRANCH
				if (prev.a < WM_MIN_PIXEL_VALUE || cur.a < WM_MIN_PIXEL_VALUE || diff.x >= _WeatherMakerTemporalReprojectionAlphaThreshold)
				{
...
#define VOLUMETRIC_MAX_HENYEY_GREENSTEIN 5.0
				...
				if (_CloudCoverVolumetric > 0.0)
				{
					...
					if (horizonFade < 1.0 && finalColor.a > 0.0)
					{
						...
						else
						{
							...
							//fixed4 backgroundSkyColor = ComputeSkySphereColorFullScreen(cloudRay, i.uv.xy + _WeatherMakerTemporalUV_FragmentShader, depth01, depth01, false, false);
		...
		Pass
		{
			...
			fixed4 frag(wm_full_screen_fragment i) : SV_Target
			{
				...
				fixed4 pixelColor = WM_SAMPLE_FULL_SCREEN_TEXTURE(_MainTex2, i.uv.xy);
				pixelColor.rgb = ComputeAtmosphericScatteringFog(i.uv.xy, WEATHER_MAKER_CAMERA_POS, GetFullScreenRayDir(i.forwardLine), length(i.forwardLine), fixed4(pixelColor.rgb, 1.0), GetDepth01(i.uv.xy)).rgb;
				return pixelColor;
			}
