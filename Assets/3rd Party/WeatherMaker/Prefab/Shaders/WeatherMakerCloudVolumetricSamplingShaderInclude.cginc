...
float SampleCloudDensityDetails(float noise, float3 marchPos, float heightFrac, float4 weatherData, float lod)
{
	...
	if (volumetricPositionDetailScale > 0.0 && noise > VOLUMETRIC_CLOUD_MIN_NOISE_VALUE && _CloudNoiseDetailPowerVolumetric > 0.0)
	{
		...
		if (volumetricPositionCurlScale > 0.0)
		{
			...
			float4 curlPos = float4(marchPos * volumetricPositionCurlScale, lod);
		...
		float heightFracType = (_CloudTypeVolumetric + cloudType) * 0.5;
		float detailPower = clamp(detailModifier * heightFracType * _CloudNoiseDetailPowerVolumetric * 2.0, _CloudNoiseDetailPowerVolumetric, 1.0);
		noise = max(0.0, (Remap(noise, detailModifier * detailPower, 1.0, 0.0, 1.0) * _CloudNoiseScalarVolumetric *
			Remap(heightFrac, 0.0, 1.0, 0.25, 0.5 + (cloudType * 0.5) + density)));
...
float GetCloudRoundness(float3 marchPos, float4 weatherData, float heightFrac)
{
	...
	else
	{
		...
		float heightFracType = min(1.0, Remap(heightFrac, 0.0, _CloudTypeVolumetric, 0.0, 1.0));
		float cloudRoundness = (1.0 - max(0.0, heightFracType - _CloudRoundnessVolumetric));
		cloudRoundness *= cloudRoundness * // rounding to top
			gradient.x * // restrict to current gradient
			(volumetricPositionShapeScale > 0.0); // we have shape scale
		...
		// top/bottom fade
		cloudRoundness *= saturate(Remap(heightFrac, 0.0, _CloudBottomFade, 0.0, 1.0)) * // round bottom
			saturate(Remap(heightFrac, _CloudTopFade, 1.0, 1.0, 0.0)); // round top
...
float SampleCloudDensity(float3 marchPos, float4 weatherData, float heightFrac, float lod, bool sampleDetails, out bool sampled)
{
	...
	if (sampled)
	{
		...
		marchPos += (volumetricWindDir2 * heightFrac);
