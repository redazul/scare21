...
fixed3 ComputeDirectionalLightCloud(float noise, float3 worldPos, float lod, int cloudIndex, CloudState state)
{
...
fixed3 ComputeCloudLighting(float noise, float3 rayDir, float3 worldPos, float2 uv, float lod, uint cloudIndex, fixed alphaAccum, CloudState state)
{
...
CloudColorResult ComputeFlatCloudColor(float3 rayDir, float3 worldPos, float2 uv,
	float depth, sampler2D noiseTex, float4 screenUV, uint cloudIndex, float lod, fixed alphaAccum, CloudState state)
{
	float3 worldPosHit;
...
CloudColorResult ComputeFlatCloudColorAll(float3 cloudRay, float depth, float4 uv, float lod, CloudState state)
{
	fixed4 cloudColor;
