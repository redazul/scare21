...
fixed4 ComputeSkySphereColorFullScreen(float3 rayDir, float2 uv, float depth01, float worldDistance01, bool doMie, bool doNight)
{
	...
	{
		depthFade = saturate(1.5 * (_WeatherMakerSkyFade.x < 0.99999) * ((((worldDistance01 - _WeatherMakerSkyFade.x) * _WeatherMakerSkyFade.y) * sunSkyFadeLerp * rayYFade)));
	}
	...
	if (depthFade > 0.00001)
	{
		...
		if (WM_ENABLE_PROCEDURAL_TEXTURED_SKY || WM_ENABLE_PROCEDURAL_SKY || WM_ENABLE_PROCEDURAL_SKY_ATMOSPHERE || WM_ENABLE_PROCEDURAL_TEXTURED_SKY_ATMOSPHERE)
		{
			...
			else
			{
				...
				fixed3 lightColor = _WeatherMakerSunColor.rgb;
				lightColor *= (_WeatherMakerSunColor.a * _WeatherMakerSunColor.a);
				skyColor.rgb = IntegrateAtmosphereScatteringSimple(WEATHER_MAKER_CAMERA_POS, rayDir, 1.0 / 0.0,
					_WeatherMakerSunDirectionUp, lightColor, doMie, transmittance);
				*/
...
fixed4 ComputeSkySphereColorVolumetric(wm_volumetric_data i, bool doMie, bool doNight)
{
	...
	return ComputeSkySphereColorFullScreen(rayDir, i.uv.xy, depth01, worldDistance01, doMie, true);
}
